...
const createAIPromptUsage = async (req, res) => {
  const { prompt_text } = req.body;
  const user_id = req.user.userId;

  // Check subscription plan and prompt limit
  db.query('SELECT us.plan_id, sp.prompt_limit, us.end_date FROM user_subscriptions us JOIN subscription_plans sp ON us.plan_id = sp.id WHERE us.user_id = ? AND us.status = "active"', [user_id], async (err, results) => {
    ...
    db.query('SELECT COUNT(*) AS prompt_count FROM ai_prompt_usage WHERE user_id = ?', [user_id], async (err, countResults) => {
      if (err) return res.status(500).send({ error: err.message });
      ...
      try {
        ...
        // if (!aiResponse) {
        //   return res.status(500).send({ message: 'AI response is empty' });
        // }
